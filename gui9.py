...
class ListItemsReachingExpiry(tk.Frame):
    def __init__(self, parent, controller):
        ...
        canvas.create_text(
            ...
            9.0,
            anchor="nw",
            text="List of Items Reaching Expiry ",
            fill="#FFFFFF",
        ...
        button_9 = Button(
            ...
            borderwidth=0,
            highlightthickness=0,
            command=lambda: [remove_expired_data()], #controller.show_current_frame(ListItemsReachingExpiry)
            relief="flat"
        )
        button_9.place(
            x=149.0,
        ...
        def remove_expired_data():
            ...
            cursor = cnx.cursor("SET SQL_SAFE_UPDATES=0")
            cursor.execute("DELETE FROM `ba4wmpjwd2fynuzr9vvq`.`fridge` WHERE `validationDate`<NOW();")
            cursor.execute("SET SQL_SAFE_UPDATES=1")
            cnx.commit()
            cnx.close()
            canvas.delete("safe_delete")
        ...
        def load_data():
            ...
            cursor = cnx.cursor()
            cursor.execute("SELECT * FROM items_expiring")
            result = cursor.fetchall()

            # Insert the result into the treeview
            current_y = 50
            for row in result:
                self.item_rectangle_1 = canvas.create_rectangle(
                    0.0,
                    current_y,
                    360.0,
                    current_y + 127,
                ...
                self.item_rectangle_2 = canvas.create_rectangle(
                    21.0,
                    current_y + 28,
                    118.0,
                    current_y + 113,
                ...
                self.item_text_1 =canvas.create_text(
                    134.0,
                    current_y + 29,
                    anchor="nw",
                    text="Item:" + row[1],
                ...
                self.item_text_2 =canvas.create_text(
                    134.0,
                    current_y + 59,
                    anchor="nw",
                    text="Quantity: " + row[7],
                ...
                self.item_text_3 =canvas.create_text(
                    134.0,
                    current_y + 89,
                    anchor="nw",
